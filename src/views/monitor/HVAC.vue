<template>
  <div class="init HVAC">
    <Condition  :conditionNum = 'conditionNum' @getList = "getHVACData"></Condition>
    <div class="house-box">
      <a href="javascript:;" :class='[ houseActive === index ? "active" : ""]' v-for = "(houseNameItem,index) in HVACData" :key = "index" @click = "getHouseData(index)">{{houseNameItem.HouseName}}</a>
    </div>
    <div class="HVAC-content">
      <ul class="environment-box">
        
        <li v-for = "(HouseEnvironments,index) in showHVACData.HouseEnvironments" :key = "index">
      ...
      <ul class="pump-box">
        <li 
        v-for = "(DisplayParams,index) in showHVACData.DisplayParams" 
        :key = "index" 
...
export default {
  ...
  methods:{
    getHouseData(index){
      ...
      this.getHVACData(this.s,this.entityID)
    },
    getHVACData(s,entityID){
      this.s = s,this.entityID = entityID;
      
      this.axios.get('http://222.180.250.162:'+this.GLOBAL.host+'/Api/V3/GetRealValue',{params:{auth:"",token:this.token,scope:this.s,entityID:this.entityID,categoryID:5}})
      .then((response)=>{
        console.log(response.data);
        this.HVACData = response.data.Rows;
        this.showHVACData = this.HVACData[this.houseActive];
        console.log(this.showHVACData.DisplayParams)
      
        //console.log(this.HVACData[this.houseActive].DisplayParams[0].DisplayItems);

      }).then(()=>{

        var voltagexAxisData = [],voltageData = [];

        for(var i = 0;i<this.HVACData[this.houseActive].DisplayParams.length;i++){
          
            voltagexAxisData[i] = [],voltageData[i] = [];
              for(var j = 0;j< 6 ;j++){
                voltagexAxisData[i].push(this.HVACData[this.houseActive].DisplayParams[i].DisplayItems[j].ItemText)
                  voltageData[i].push(this.HVACData[this.houseActive].DisplayParams[i].DisplayItems[j].DisplayText);
              }
